...
Sources: `docs/uhdi-spec.md` (Sec.14 open questions, Sec.15 projections, Appendix B.9-B.11),
`docs/uhdi-action-plan.md` (Sec.3.4.1, Sec.4.3, Sec.5, Sec.6, Sec.7), `bench/README.md`
...
of Sec.1.2 "superset of three legacy formats" -- collapses. The full argument is in
Appendix B.9 (`docs/uhdi-spec.md:3072-3078`) and in `docs/uhdi-action-plan.md:30-38`.
This is visible from Sec.15: HGLDD is one of three projections on equal footing with
...
Sec.10 dataflow is fully specified in the spec but not implemented in the emitter.
...
Provenance (historically Sec.12) has been removed entirely from the spec in 0.9.3 --
no emitter produced the corresponding pool, no consumer read it, so the layer was
cut along with temporal (Sec.11) and reserved in git history until a first real
...
create a fourth silo, which directly contradicts the Sec.1.1 problem statement.

### 1.5 "What is `<complex>` in `enableRef`, and is the `&`-joined string a hack?"

This is the transitional MVP form of Sec.9.3, explicitly marked as transitional in the
spec (`docs/uhdi-spec.md:1512-1514`) and in action plan Sec.3.4.1
...
requires C++ MLIR work in `circt:fk-sc/uhdi-pool` (~1/2 day emitter + 1/2 day
...
linter (Sec.13, `docs/uhdi-spec.md:2646+`) excludes the MVP form from the
every-`*Ref`-resolves rule.

### 1.6 "What is `representations`, and why an N-way map instead of an HGL/HDL pair?"

`representations` (Sec.3.2, `docs/uhdi-spec.md:281-289`) is a map declaring all IR
...
semantic mapping roles are handled via Sec.3.3
...
`Instance`/`Variable`/`Breakpoint`/`Generator Variable` (Sec.15.4.1,
...
`(fixture x target)` cell before the structural diff; (c) `docs/uhdi-spec.md`
quotes schema snippet subsets in every Sec.N.4 so that the spec and schema files
...
Sec.15: ingestion requires auxiliary inputs (VCD, FIRRTL dump) for type-width recovery
in two out of three cases (Sec.15.1, `docs/uhdi-spec.md:2738`). Sec.15.6 formalizes the
...
| R5 | Sec.9.3 MVP `&`-joined predicate string + sentinel `<complex>` -- transitional, schema-legal, but may look like a hack under deep probing by a reviewer. Does not block M1/M2. | P1 | Answer Q1.5 is prepared; action plan Sec.3.4.1 (`docs/uhdi-action-plan.md:318-330`) formalizes the post-defense rewrite. | author / answered |
| R6 | Sanity checks **A13-A16** are listed as "pre-sanity" in action plan Sec.1.1-1.4, but the status table Sec.0 (`docs/uhdi-action-plan.md:13-19`) shows [ok] only for A1-A4. If A15 (Tywaves on HGLDD baseline) and A16 (Chisel `withDebug` -> `circt_debug_*` intrinsics) fail, Phase 1 demo silently degrades. | P1 | Re-run A13-A16 on the current checkout (A13 trivial, A14 already reflected by the emitter's existence, A15/A16 require tywaves + scala-cli locally). Update the Sec.0 table. | author / unverified |
| R7 | Assumptions A5-A8 in Sec.5 (`docs/uhdi-action-plan.md:432-441`) are listed as "being verified" / "being resolved": A5 stable IDs deterministic, A6 capture-when conflict-free, A7 uhdi-attrs survive pipeline (==A13), A8 SourceInfo preserved. Without an explicit green flag they all carry P2 risk. | P2 | A5 -> bench `manifest.toml` shows determinism by hash; A6 -> emitter works on 5 demos without failures -- build the argument in evaluation; A7 == A13; A8 -> SourceInfo is used in the Tywaves screenshot. | author / inferentially-passed |
...
| R9 | Sec.3.4.1 post-defense workstream (long-term `enableRef` shape) -- fork at `circt:fk-sc/uhdi-pool` remains open. Does not block the defense, but a reviewer may ask "will this actually get closed?". | P2 | Q1.5 explains; action plan Sec.3.4.1 explicitly scopes the work: ~1 day C++ + fixture regeneration. | author / scheduled |
...
|------|----------------|--------------|-----------------|-----------------------|--------------|
| `demo/gcd/` | [ok] symlink -> `demo/run.sh` | [ok] `--with-experimental-debug-intrinsics` | UInt arithmetic, single module, simplest end-to-end. README hgdb console session (`demo/README.md:151-205`) uses GCD. | **Likely hgdb-slide proxy** (deck slide is titled "Demo on Counter", but the actual demo repo has GCD as the most polished path). Also suitable for the Tywaves slide set. | **MUST WORK.** `design.db` exists (40 KB, May 8); contents for multi-arm `when` branches **not verified** -- see R3. Run `./run.sh build` + `hgdb-db design.db` by hand. |
| `demo/fsm/` | [ok] symlink | [ok] verified | `ChiselEnum`-FSM (TrafficLight); Tywaves should show state as `Red/RedYellow/Green/Yellow` (`demo/README.md:30`). | Tywaves enum-rendering -- potential tywaves slide. | Enum projection stress-test **not run** -- R4. Run `./run.sh simulate` + tywaves manually. |
| `demo/fifo/` | [ok] symlink | [ok] verified | `Decoupled<UInt>` + `SyncReadMem`; Bundle ports collapse into Tywaves struct view. | Tywaves bundle-rendering -- potential tywaves slide. | Bundle projection in HGLDD broken (R2) -- `io_*` fields may not appear in `objects[]`. |
| `demo/pipeline/` | [ok] symlink | [ok] verified | 3-stage MAC + two sub-Modules; hierarchy navigation in Tywaves; hgdb step across pipeline registers. | Potential hgdb-slide or Tywaves-hierarchy slide. | Single-when only inside stages -- hgdb path should work without multi-arm gap. Smoke manually. |
| `demo/bus/` | [ok] symlink | [ok] verified | `Decoupled` of nested `Bundle` (`Request{addr,data,write}` -> `Response{data,ok}`). | Nested-record stress; not explicitly in the deck. | Bundle projection broken (R2) -- two levels of nesting worsen the gap. |
...
- **`README.md:73`** (`uv pip install ... --no-config`) -- a working path, but
  ...
  worth clarifying.
- **`docs/uhdi-action-plan.md:13-19`** -- the table shows [ok] only for A1-A4.
  A13-A16 are described in Sec.1.1-1.4 as "being verified", but in the final table
  Sec.5 (`docs/uhdi-action-plan.md:432-445`) their status is `Pre-sanity (Sec.1.x)`.
  **Run A13-A16 and mark [ok] or unresolved** before the defense -- otherwise a
  reviewer will point out the gap. See R6.
- **`docs/uhdi-action-plan.md:308-316`** (Sec.3.4 Emitter extension) and
  **Sec.3.4.1** (`docs/uhdi-action-plan.md:318-330`) -- added by the current audit.
  The body of Sec.3.4 describes the MVP, Sec.3.4.1 describes the long-term shape.
  They are in the same section, but Sec.3.4 does not say "the current implementation
  is the MVP, see Sec.3.4.1". Worth adding a one-liner cross-reference.
- **`docs/uhdi-action-plan.md:511-515`** (Sec.7.3 anticipated questions) -- four
  questions are listed but **no answers are given in the action plan**. This file
  (`docs/defense-readiness.md` Sec.1) closes them. Worth adding a cross-reference
  from Sec.7.3 to here.
- **`bench/README.md:130-152`** "Pending fixtures" -- the section is current
  ...
  make it into action plan Sec.5 "Summary of assumptions". Worth adding them there
...
1. **T-21 -> T-18: Close P0 risks R1/R2/R3 (demo semantic readiness).**
   1. Fix the deck reference in `solution/counter-demo.html`: either point it to
      ...
      gcd structure. Record the decision in `docs/uhdi-action-plan.md` Sec.3.6 / Sec.7.
...
2. **T-18 -> T-14: A13-A16 sanity checks (R6).**
   ...
   4. A16: `withDebug` is present in Chisel sims
      ...
      verified by grep.
   - Mark [ok] in `docs/uhdi-action-plan.md:13-19`.
...
4. **T-10 -> T-7: Documentation final pass.**
   1. README.md `--no-config` clarification (gap Sec.4 item 1).
   2. Action plan Sec.0 [ok]-table updated (gap Sec.4 item 2).
   3. Action plan Sec.3.4 cross-reference to Sec.3.4.1 (gap Sec.4 item 3).
   4. Action plan Sec.7.3 cross-reference to defense-readiness.md Sec.1 (gap Sec.4 item 4).
   5. bench/README.md Pending fixtures cross-reference to risk register
      (gap Sec.4 item 5).
   6. tools/README.md image tag sanity check (gap Sec.4 item 6).
   7. demo/README.md hgdb session reviewed against current output (gap Sec.4 item 7).

5. **T-7 -> T-3: Slides + speaker notes.**
   1. Speaker note for action plan Sec.4.3 (`docs/uhdi-action-plan.md:416-424`) --
      finalize.
   2. 4 "obvious" questions (Sec.7.3) -> answers from Sec.1 of this document.
...
6. **T-3 -> T-1: Rehearse aloud at least 2 times.** Action plan Sec.7.3 last bullet
