...
### 1.2 «Почему не dataflow в текущей версии? Что случилось с provenance?»

§10 dataflow специфицирован в спеке полностью, но не реализован в
эмиттере. Это сознательный scope cut, зафиксированный в
`docs/uhdi-action-plan.md:45` («никакого Phase 3+ задела»): pool-based
JSON и три проектора (HGLDD / hgdb / PDG) демонстрируют формат,
dataflow-граф ничего не добавляет к этой демонстрации и требует
отдельной producer-инструментации в CIRCT. Честный ответ: это future
work, spec задаёт фундамент, реализация — следующая итерация после
Phase 2.

Provenance (исторически §12) полностью удалён из spec в 0.9.3 — ни один
эмиттер не производил соответствующий pool, ни один консьюмер не читал
его, поэтому слой был вырезан вместе с temporal (§11) и зарезервирован
в git history до момента, когда появится первый реальный producer или
consumer. См. changelog `docs/uhdi-spec.md` запись «0.9.3 (2026-05-21)».
